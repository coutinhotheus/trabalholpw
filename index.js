...
const produtos = [{idProd: '2020', nome: 'bone', quantidade: 10, valorUnitario: 20}]
...
const modificador = (request, response, next) =>{
...
app.put("/produtos/:id", modificador, (request, response) =>{
    const { id } = request.params
    const prod = request.body
    const {idProd, nome, quantidade, valorUnitario} = prod
    prod.pTotal = quantidade*valorUnitario
...
app.delete("/produtos/:id" , modificador, (request, response) => {
    const { id } = request.params
